...
// TODO @AI：改成 member/index.ts，参考别的 api ts；
...
export const CalTeamMemberApi = {
  ...
  getTeamMemberListByTeam: async (teamId: number) => {
    return await request.get({ url: `/mes/cal/team-member/list-by-team?teamId=` + teamId })
  },

  // 查询多个班组的成员列表
  getTeamMemberListByTeamIds: async (teamIds: number[]) => {
    return await request.get({ url: `/mes/cal/team-member/list-by-team-ids`, params: { teamIds: teamIds.join(',') } })
  }
}
